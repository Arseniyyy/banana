...
if __name__ == '__main__':
    ...
    sock1 = sock('192.168.4.218', 50007)
    sock1.sock_setup()
...
#       cv.imshow("Lane", out_img)
            ...
            if np.sum(rightx, 0) == 0 | np.sum(righty, 0) == 0:
                ...
                for ver_ind in range(out_img.shape[0]):
                    ...
                    if ver_ind == int(out_img.shape[0] // 2):
                        center_point = gor_ind
                        derivative = 2*(center_fit[0])+center_fit[1] * ver_ind
                        #main_high_x = gor_ind
                        #main_high_y = ver_ind
                        dx = -(midpoint - center_point)
                        pass
        ...
        dt = time.time() - st
        #angle_rad = math.atan(derivative)
        ...
        Uf, Pf, If, Df = PID(speed_ref, dvy_f, 1, -1, 0.02, 6, 0.015, Integral=If, dt=dt)
        Uan, Pan, Ian, Dan = PID(0, dx, 1.6, -1.6, 0.0, 0.1, 0.0, Integral=Ian, dt=dt)
        angle_rad = Uan
